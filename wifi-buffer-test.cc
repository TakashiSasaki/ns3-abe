...
static void GenerateTraffic (Ptr<Socket> p_socket, uint32_t pktSize, const Address& destination_address)
{
  Ptr<Node> p_node = p_socket->GetNode(); 
  uint8_t uint8_buffer[pktSize];
  for(unsigned int i=0; i<pktSize; ++i) uint8_buffer[i] = 0;
...
int main (int argc, char *argv[])
{
  ...
  Ptr<Node> p_source_node =  node_container.Get(0);
  Ptr<Node> p_destination_node = node_container.Get(3);
  const Ptr<Ipv4L3Protocol> p_source_ipv4_l3_protocol = p_source_node->GetObject<Ipv4L3Protocol>();
...
#endif
    Simulator::ScheduleWithContext (p_source_node->GetId (),
                                    Seconds (t += interval), &GenerateTraffic, 
                                    p_source_raw_socket, packetSize, destination_address);
