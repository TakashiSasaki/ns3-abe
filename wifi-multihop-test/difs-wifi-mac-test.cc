...
class DifsWifiMacTest {
	...
	void receivePacket(ns3::Ptr<ns3::Socket> p_socket) {
		...
		this->outputFileStream << ns3::Simulator::Now() << " "
				<< p_socket->GetNode()->GetId() << " receive "
				<< tag.GetFlowId() << std::endl;
	...
	void sendPacket(ns3::Ptr<ns3::Socket> p_socket) {
		ns3::Ptr<ns3::Packet> p_packet = ns3::Create<ns3::Packet>(
				this->packetSize);
		...
		this->outputFileStream << ns3::Simulator::Now() << " "
				<< p_socket->GetNode()->GetId() << " send " << flowId
				<< std::endl;
	}//sendPacket
	...
	void initIpv4Interface() {
		...
		ns3::Ipv4InterfaceContainer i = ipv4_address_helper.Assign(
				this->nodeContainer.getNetDeviceContainer());
	}//initIpv4Interface
	...
	ns3::Ptr<ns3::Socket> getIpv4RawSocket(const int i_node) {
		...
		ipv4_raw_socket->SetRecvCallback(ns3::MakeCallback(
				&abe::DifsWifiMacTest::receivePacket, this));
...
public:
	DifsWifiMacTest(const std::string phyMode = std::string("DsssRate11Mbps"),
		...
		phyMode(phyMode), packetSize(packetSize), lambda(lambda),
	...
	void scheduleSendPacket(const ns3::Time& t, const int i_node) {
		ns3::Simulator::Schedule(t, &abe::DifsWifiMacTest::sendPacket, this,
				this->getIpv4RawSocket(0));
	}//scheduleSendPacket
	...
	void schedule() {
		...
		for (int n = 0; n < this->nodeContainer.GetN() - 1; n++) {
			do {
				this->scheduleSendPacket(ns3::Seconds(send_time
