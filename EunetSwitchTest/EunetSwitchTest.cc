...
int preparesimulation(int argc,char *argv[]){
    ...
    OnOffHelper on_off_helper("ns3::UdpSocketFactory", Address(InetSocketAddress(Ipv4Address("10.1.1.2"), port)));
    on_off_helper.SetConstantRate(DataRate("500kb/s"));
    on_off_helper.SetAttribute("Remote", AddressValue(InetSocketAddress(Ipv4Address("10.1.1.1"), port)));
